...
//流量判断部分
	...
	console.log("通用变化量:"+limitChange)
  	if(limitChange!=0){$persistentStore.write(String(limitusagetotal),"limitStore")}  //进行判断是否将本次查询到的值存到本地存储器中供下次使用
  	if(unlimitChange!=0){$persistentStore.write(String(unlimitusagetotal),"unlimitStore")}  //进行判断是否将本次查询到的值存到本地存储器中供下次使用
//*******
	...
	{
		$persistentStore.write(String(unlimitusagetotal),'unlimittoday')
		$persistentStore.write(String(limitusagetotal),'limittoday')
	}
...
{	
	...
	{  	
		...
		{
			$persistentStore.write(String(thishours),"hourstimeStore")
			$persistentStore.write(String(thisminutes),"minutestimeStore") 
