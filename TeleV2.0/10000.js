...
function Env(name) {
	...
	SH = typeof $environment && $environment['stash-version']!== `undefined`
	read = (key) => {
	  if (LN || SG || SH) return $persistentStore.read(key)
	  if (QX) return $prefs.valueForKey(key)
	}
	write = (key, val) => {
	  if (LN || SG || SH) return $persistentStore.write(key, val); 
	  if (QX) return $prefs.setValueForKey(key, val)
	}
	notice = (title, subtitle, message, url) => {
	  if (LN) $notification.post(title, subtitle, message, url)
	  if (SG || SH) $notification.post(title, subtitle, message, { url: url })
	  if (QX) $notify(title, subtitle, message, { 'open-url': url })

	}
	get = (url, cb) => {
	  if (LN || SG || SH) {$httpClient.get(url, cb)}
	  if (QX) {url.method = `GET`; $task.fetch(url).then((resp) => cb(null, {}, resp.body))}
	}
	post = (url, cb) => {
	  if (LN || SG || SH) {$httpClient.post(url, cb)}
